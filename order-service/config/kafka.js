...
export const publishMessage = async (producer, topic, message, key = null) => {
  try {
    ...
    console.log(`🔍 [kafka] Publishing to ${topic}:`, serializedMessage.substring(0, 200) + '...');
    return producer.send({ topic, messages: [{ key: key || null, value: serializedMessage }] });
  } catch (error) {
    console.error(`❌ [kafka] Failed to serialize message for topic ${topic}:`, error.message);
    console.error(`❌ [kafka] Message that failed to serialize:`, message);
    throw error;
  }
};
